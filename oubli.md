...
The cryptographic and protocol layer is provided by the [`kms`](/Users/adrienlacombe/ws/kms) monorepo's **Rust crates** (`crates/core/*`), consumed as normal Cargo dependencies by Oubli's Rust workspace. Oubli adds the mobile hardening layer (auth, storage, backup, session) in Rust on top, then exposes a single FFI surface to native UI shells via UniFFI.
...
| Starknet RPC + address derivation | `starknet-client` (`crates/core/starknet-client`) | OZ account address, Pedersen hash chain |
